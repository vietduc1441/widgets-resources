    ...
    return {
        ...
        async resolveId(source) {
            ...
            try {
                ...
                const packageContent = await promises.readdir(packageDir);
                const hasNativeCode = packageContent.some(fileName => /^(ios|android|.*\.podspec)$/i.test(fileName));
                if (hasNativeCode) {
...
function traceMe(prefix) {
    return {
        name: "trace",
        resolveId: async (source, importer) => {
            console.log(prefix, source, importer);
            return null;
        }
    };
}
